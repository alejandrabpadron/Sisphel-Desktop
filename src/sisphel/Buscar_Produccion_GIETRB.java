...
class Buscar_Produccion_GIETRB {
    ...
    void bpro(String valor, String filtro, JTable tbl_produccion) {
       String[] columnas = {"AUTORES", "TÍTULO", "JOURNAL - REVISTA,", "TIPO", "AÑO","URL"};
        ...
        if (filtro.equals("Autores")) {
            sql = ("SELECT Autores,Titulo,Journal_Revista,Tipo,Año,URL FROM produccion_cientifica WHERE Grupo='02' && Autores LIKE '%" + valor + "%'");

        } else if (filtro.equals("Título")) {
            sql = ("SELECT Autores,Titulo,Journal_Revista,Tipo,Año,URL FROM produccion_cientifica WHERE Grupo='02' && Titulo LIKE '%" + valor + "%'");

        } else if (filtro.equals("Revista - Journal")) {
            sql = ("SELECT Autores,Titulo,Journal_Revista,Tipo,Año,URL FROM produccion_cientifica WHERE Grupo='02' && Journal_Revista LIKE '%" + valor + "%'");

        } else if (filtro.equals("Tipo de Publicación")) {
            sql = ("SELECT Autores,Titulo,Journal_Revista,Tipo,Año,URL FROM produccion_cientifica WHERE Grupo='02' && Tipo LIKE '%" + valor + "%'");

        } else {
            sql = ("SELECT Autores,Titulo,Journal_Revista,Tipo,Año,URL FROM produccion_cientifica WHERE Grupo='02' && Año LIKE '%" + valor + "%'");
