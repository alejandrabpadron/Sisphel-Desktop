package sisphel;

import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.SQLException;
import javax.swing.JOptionPane;
import javax.swing.ImageIcon;

public class Mod_Equipos {

    Conexion con = new Conexion();

    public void me(String piu, String pig, String equi, String marca, String area, String Id) {
        int confirmar = JOptionPane.showConfirmDialog(null, "¿Desea modificar los datos actuales?");
        if (confirmar == JOptionPane.YES_OPTION) {

            Connection c = null;
            try {
                c = con.conexion();
                String sql = ("UPDATE equipos SET Placa_Unisinu=?, Placa_Gobernacion=?, Descripcion=?, Marca=?, Area=? WHERE Id=?");
                PreparedStatement psql = c.prepareStatement(sql);
                psql.setString(1, piu);
                psql.setString(2, pig);
                psql.setString(3, equi);
                psql.setString(4, marca);
                psql.setString(5, area);
                psql.setString(6, Id);

                if (psql.executeUpdate() > 0) {
                    JOptionPane.showMessageDialog(null, "Operación Exitosa \nLos datos se han modificado correctamente", "SISPHEL - [Modificar Equipo]", JOptionPane.INFORMATION_MESSAGE, new ImageIcon("src/img/check.png"));

                } else {
                    JOptionPane.showMessageDialog(null, "Operación Fallida\nError al modificar la información", "SISPHEL - [Modificar Equipo]", JOptionPane.ERROR_MESSAGE, new ImageIcon("src/img/error.png"));

                }
            } catch (SQLException e) {
                JOptionPane.showMessageDialog(null, "***ERROR***\nNo se ha podido realizar la actualización de los datos" + e, "SISPHEL - [ERROR]", JOptionPane.ERROR_MESSAGE, new ImageIcon("src/img/error.png"));

            } finally {
                try {
                    if (c != null) {
                        c.close();
                    }
                } catch (SQLException e) {
                    JOptionPane.showMessageDialog(null, "Error al Cerrar La Conexión:\n" + e, "Error En La Operacion", JOptionPane.ERROR_MESSAGE);
                }
            }
        }
    }
}
