...
public class Buscar_Proyectos_GIBM {
    ...
    void bpro(String valor, String filtro, JTable tbl_proyectos) {
        ...
        if (filtro.equals("Estado")) {
            sql = ("SELECT h.Nombre, h.Estado FROM proyectos h WHERE  h.Grupo='01' && h.Estado LIKE '%" + valor + "%'");

        } 
        else{
            sql = ("SELECT h.Nombre, h.Estado FROM proyectos h WHERE h.Grupo='01' && h.Nombre LIKE '%" + valor + "%'");
